...
{
    unsigned long stringLength = [base64String length];
...
NSString *rsaSignString(NSString *stringToSign, NSString *privateKeyFilePath, BOOL *signSuccess)
{
    ...
    int sign_ok = rsa_sign_with_private_key_pem((char *)message, (int)messageLength
                                                , sig, &sig_len
                                                , filePath);
...
void rsaVerifyString(NSString *stringToVerify, NSString *signature, NSString *publicKeyFilePath, BOOL *verifySuccess)
{
    ...
    unsigned long messageLength = [stringToVerify lengthOfBytesUsingEncoding:NSUTF8StringEncoding];
    NSData *signatureData = dataFromBase64String(signature);
    unsigned char *sig = (unsigned char *)[signatureData bytes];
    unsigned long sig_len = [signatureData length];
...
{    
    ...
    unsigned long len = [trimmedString length];
